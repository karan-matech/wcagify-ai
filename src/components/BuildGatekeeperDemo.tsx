...
export const BuildGatekeeperDemo: React.FC = () => {
  const [gateStatus, setGateStatus] = useState<'idle' | 'scanning' | 'blocked' | 'fixed'>('idle');
  ...
  return (
    <section id="gatekeeper" className="py-12 sm:py-20 lg:py-24 bg-white border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-10 sm:mb-14">
          ...
          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight">
            Stop Inaccessible Content <span className="text-rose-600">Before It Goes Live</span>
          </h2>
          <p className="text-sm sm:text-base lg:text-lg text-slate-600 leading-relaxed">
            Think of WCAGify as an automatic security guard for your website. If anyone on your team accidentally uploads broken buttons or unreadable documents, WCAGify stops the update instantly and fixes it before customers see it.
          </p>
        </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pb-6 mb-6 border-b border-slate-800">
              ...
              <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider flex items-center gap-1.5 ${
                ...
                {gateStatus === 'fixed' ? (
                  ...
                  <>
                    ...
                    <span>Guard Status: Update Blocked (Inaccessible Content Detected)</span>
                  </>
                ) : (
                  <>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span className="hidden sm:inline">{showTechCode ? 'Hide Logs' : 'Developer View'}</span>
              </button>
            </div>
          </div>
            ...
            <div className="lg:col-span-7 bg-slate-950 p-5 sm:p-8 rounded-2xl border border-slate-800 space-y-6 relative overflow-hidden">
              ...
              <div className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-900 border border-slate-800">
                ...
                <div className="min-w-0 flex-1">
                  <span className="block text-sm font-bold text-white">Team Member Submits Web Update</span>
                  <p className="text-xs text-slate-400 mt-0.5">
                    A developer or marketer pushes new marketing pages and PDF reports to the server.
                  </p>
                </div>
              </div>
              ...
              <div className={`flex items-start gap-3 p-3.5 rounded-xl transition-all border ${
                gateStatus === 'blocked'
                ...
                <div className="min-w-0 flex-1 space-y-2">
                  ...
                  {gateStatus === 'idle' && (
                    <p className="text-xs text-slate-400">
                      Click "Simulate Publishing Inaccessible Page" above to trigger the guard.
                    </p>
                  )}

                  {gateStatus === 'blocked' && (
                    <div className="space-y-1.5 text-xs text-rose-300 bg-rose-950/60 p-3 rounded-lg border border-rose-900/50">
                      <p className="font-bold text-rose-200">Barriers Found & Stopped:</p>
                      <ul className="space-y-1">
                        ...
                        <li className="flex items-center gap-1.5">
                          <X className="w-3.5 h-3.5 text-rose-400 shrink-0" />
                          <span>Unlabelled image with no screen reader text</span>
                        </li>
                        <li className="flex items-center gap-1.5">
                          <X className="w-3.5 h-3.5 text-rose-400 shrink-0" />
                  ...
                  {gateStatus === 'fixed' && (
                    <div className="space-y-1.5 text-xs text-emerald-300 bg-emerald-950/60 p-3 rounded-lg border border-emerald-900/50">
                      <p className="font-bold text-emerald-200">AI Repairs Applied in 1.2s:</p>
                      <ul className="space-y-1">
                        <li className="flex items-center gap-1.5">
                          <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                          <span>Converted broken buttons to native HTML buttons</span>
                        </li>
                        <li className="flex items-center gap-1.5">
                          <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                          <span>Generated accurate AI alt-text descriptions</span>
                        </li>
                        <li className="flex items-center gap-1.5">
                          <Check className="w-3.5 h-3.5 text-emerald-400 shrink-0" />
                          <span>Injected PDF/UA-1 tags into uploaded documents</span>
                        </li>
                      </ul>
                    </div>
                  )}
              ...
              <div className="flex items-start gap-3 p-3.5 rounded-xl bg-slate-900 border border-slate-800">
                ...
                <div className="min-w-0 flex-1">
                  <span className="block text-sm font-bold text-white">Live Customer Site</span>
                  <p className="text-xs text-slate-400 mt-0.5">
                    {gateStatus === 'fixed'
            ...
            <div className="lg:col-span-5 space-y-6">
              ...
              <div className="p-6 bg-slate-950 rounded-2xl border border-slate-800 space-y-4">
                ...
                <p className="text-xs sm:text-sm text-slate-300 leading-relaxed">
                  Most accessibility lawsuits happen when team members publish new content or PDF files without checking accessibility.
                  WCAGify guarantees you never make a mistake by acting as a zero-effort safety filter.
                </p>
                ...
                <div className="pt-2">
                  ...
                  >
                    ...
                    <span>{isPlayingAudio ? 'Speaking Audio Status...' : '🔊 Listen to Guard Voice Audio'}</span>
                  </button>
                </div>
              </div>
              ...
              <div className="p-4 bg-slate-950 rounded-2xl border border-slate-800 flex items-center gap-3">
                ...
                <div>
                  <span className="block text-xs font-bold text-white">Fail-Closed Security Guarantee</span>
                  <span className="text-xs text-slate-400">Zero non-compliant releases reach customers.</span>
                </div>
              </div>
          ...
          {showTechCode && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-400">GitHub Actions / Vercel Build Logs</span>
              </div>
              ...
              <div className="p-4 bg-slate-950 font-mono text-xs text-slate-300 rounded-xl border border-slate-800 space-y-2 overflow-x-auto">
                <div className="text-slate-500">$ wcagify-gatekeeper check --target=./dist --standard=WCAG22AA</div>
                <div className="text-indigo-300"> Parsing AST source files, DOM tree, and PDF/UA tag matrices...</div>
                {gateStatus === 'blocked' && (
                  <div className="text-rose-400 font-bold">
                    [BUILD HALTED] FAIL-CLOSED GATEKEEPER TRIGGERED (4 Compliance Violations Found)
                  </div>
                )}
                {gateStatus === 'fixed' && (
                  <div className="text-emerald-400 font-bold">
                    [BUILD PASSED] NATIVE REMEDIATION SUCCESSFUL (0 Violations Remaining)
                  </div>
                )}
              </div>
            </motion.div>
