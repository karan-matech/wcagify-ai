...
import { Send, CheckCircle2, ShieldCheck, AlertCircle, Sparkles } from 'lucide-react';
...
export const DemoRequestForm: React.FC = () => {
  ...
  const validate = () => {
    ...
    if (!formData.details.trim()) {
      newErrors.details = 'Please briefly outline your digital asset remediation volume.';
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      setSubmitted(true);
      setLiveAnnouncement('Demo request submitted successfully. A WCAGify accessibility engineer will reach out within 24 hours.');
    } else {
      setLiveAnnouncement('Form submission failed due to validation errors. Please fix highlighted fields.');
    }
  };

  return (
    <section id="demo-request" className="py-16 lg:py-24 bg-white border-b border-slate-200">
        ...
        <div className="max-w-3xl mx-auto bg-slate-50 p-8 sm:p-10 rounded-2xl border border-slate-200 shadow-lg space-y-8">
          ...
          <div className="text-center space-y-3">
            ...
            <p className="text-sm text-slate-600 max-w-xl mx-auto">
              Request a live demonstration of the WCAGify AI engine on your organization's websites, PDFs, EPUB publications, or enterprise docs.
            </p>
          </div>
          ...
          {submitted ? (
            <div className="p-8 bg-emerald-50 border border-emerald-200 rounded-xl text-center space-y-4 animate-in fade-in duration-200">
              ...
              <p className="text-sm text-emerald-800 max-w-lg mx-auto">
                Thank you, <strong>{formData.fullName}</strong>. A WCAGify digital accessibility architect will contact you at <strong>{formData.email}</strong> to set up your tailored transformation demo.
              </p>
              <div className="pt-2">
                <button
                  type="button"
                  onClick={() => {
                    setSubmitted(false);
                    setFormData({ fullName: '', email: '', orgType: 'enterprise', assetScope: 'all', details: '' });
            ...
            <form onSubmit={handleSubmit} noValidate className="space-y-6">
              ...
              <div className="space-y-1.5">
                ...
                />
                <span id="fullName-help" className="text-xs text-slate-700 font-medium block">
                  Your primary contact name for scheduling.
                </span>
                {errors.fullName && (
                  <span id="fullName-error" role="alert" className="text-xs font-bold text-rose-600 flex items-center gap-1 mt-1">
              ...
              <div className="space-y-1.5">
                ...
                />
                <span id="email-help" className="text-xs text-slate-700 font-medium block">
                  Please use your corporate, government, or institutional email.
                </span>
                {errors.email && (
                  <span id="email-error" role="alert" className="text-xs font-bold text-rose-600 flex items-center gap-1 mt-1">
                    <AlertCircle className="w-3.5 h-3.5" aria-hidden="true" />
                    {errors.email}
                  </span>
                )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-1.5">
                  <label htmlFor="orgType" className="block text-sm font-bold text-slate-900">
                    Organization Type
                  </label>
                  <select
                    id="orgType"
                  ...
                  >
                    <option value="enterprise">Enterprise SaaS / Tech Company</option>
                    <option value="government">Government / Public Sector Entity</option>
                    <option value="publishing">Publishing House / Media Group</option>
                    <option value="education">Higher Education / Academic Institution</option>
                    <option value="healthcare">Healthcare / Financial Services</option>
                  </select>
                </div>

                <div className="space-y-1.5">
                  <label htmlFor="assetScope" className="block text-sm font-bold text-slate-900">
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({ ...formData, assetScope: e.target.value })}
                    className="w-full px-4 py-3 rounded-lg border border-slate-300 text-slate-900 bg-white focus-visible:ring-2 focus-visible:ring-indigo-600 focus-visible:ring-offset-2 outline-none"
                  >
                    <option value="all">Universal (Web, PDFs, EPUB, Docs)</option>
                    <option value="web">Websites & SaaS Web Apps</option>
                    <option value="pdfs">Enterprise Tagged PDFs (PDF/UA)</option>
                    <option value="epub">EPUB 3 E-Books & Journals</option>
                    <option value="knowledge">Internal Knowledge Portals</option>
                  </select>
                </div>
              ...
              <div className="space-y-1.5">
                <label htmlFor="details" className="block text-sm font-bold text-slate-900">
                  Asset Volume & Remediation Timeline <span className="text-rose-600" aria-hidden="true">*</span>
                </label>
                <textarea
                  id="details"
                  name="details"
                  ...
                  className={`w-full px-4 py-3 rounded-lg border text-slate-900 bg-white transition-colors focus-visible:ring-2 focus-visible:ring-indigo-600 focus-visible:ring-offset-2 outline-none ${
                    errors.details ? 'border-rose-500 bg-rose-50/20' : 'border-slate-300'
                  }`}
                  placeholder="e.g. We have ~5,000 untagged annual PDFs and a Next.js web portal that needs EAA EN 301 549 compliance by Q2..."
                />
                <span id="details-help" className="text-xs text-slate-700 font-medium block">
                  Helps our engineers prepare relevant live transformation samples for your demo.
                </span>
                {errors.details && (
                  <span id="details-error" role="alert" className="text-xs font-bold text-rose-600 flex items-center gap-1 mt-1">
                    <AlertCircle className="w-3.5 h-3.5" aria-hidden="true" />
                    {errors.details}
                  </span>
                )}
