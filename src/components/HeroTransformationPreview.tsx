...
export const HeroTransformationPreview: React.FC = () => {
  const [activePersona, setActivePersona] = useState<PersonaMode>('screenreader');
  ...
  // Auto-scan trigger when user switches modes or clicks "Run AI Fix"
  ...
  return (
    <section id="pipeline" className="py-12 sm:py-20 lg:py-24 bg-white border-b border-slate-200">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
        ...
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-10 sm:mb-14">
          ...
          <h2 className="text-2xl sm:text-4xl lg:text-5xl font-extrabold text-slate-900 tracking-tight leading-tight">
            How WCAGify Fixes Your Website <span className="text-indigo-600">In Seconds</span>
          </h2>
          
          <p className="text-sm sm:text-base lg:text-lg text-slate-600 leading-relaxed">
            No complicated code knowledge required. See how WCAGify automatically removes barriers for real people—whether they use screen readers, keyboard navigation, or high-contrast display modes.
          </p>
        </div>

        {/* Persona Simulator Selector Buttons */}
        ...
        {/* Animated Interactive Card Display Container */}
        <div className="bg-slate-900 rounded-3xl p-4 sm:p-8 lg:p-10 shadow-2xl border border-slate-800 text-white max-w-5xl mx-auto relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
            ...
            {/* Simulated Web Card - Visual Representation */}
              ...
              <div className="space-y-5">
                ...
                <div className="space-y-1">
                  <h3 className={`text-xl font-bold transition-all ${
                    !isTransformed && activePersona === 'contrast'
                      ? 'text-slate-600/40' // Hard to read low contrast
                      : 'text-white'
                  }`}>
                ...
                <div className="relative rounded-xl overflow-hidden bg-slate-900 border border-slate-800 p-4 flex items-center gap-4">
                  ...
                  <div className="space-y-1 min-w-0">
                    <p className="text-sm font-semibold text-slate-200">Order Summary Chart</p>
                  ...
                  <label className="block text-xs font-medium text-slate-300">
                    {isTransformed ? (
                      <span className="flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-emerald-400 font-bold">Explicit &lt;label&gt; tag</span>
                      </span>
                    ) : (
                      <span className="flex items-center justify-between text-rose-400">
                        <span>(Unlinked label div)</span>
                        <span className="text-[10px] font-bold">Unlabelled Input</span>
                      </span>
                    )}
                  </label>
                  ...
                  <input
                    ...
                    className={`w-full px-3.5 py-2.5 rounded-xl bg-slate-900 text-sm text-slate-200 border transition-all ${
                      activePersona === 'keyboard' && isTransformed
                        ...
                        : activePersona === 'keyboard' && !isTransformed
                        ? 'border-slate-800 ring-0 opacity-50' // Unreachable via keyboard
                        : 'border-slate-800'
                    }`}
                  />
                  {activePersona === 'keyboard' && (
                    <p className={`text-xs ${isTransformed ? 'text-emerald-400' : 'text-rose-400'}`}>
                      {isTransformed
                  ...
                  {!isTransformed ? (
                    <div className="p-3 bg-slate-900 border border-rose-900/50 rounded-xl text-center cursor-not-allowed">
                      <span className="text-xs text-rose-400 font-mono block">
                        &lt;div class="btn" onclick="..."&gt;Complete Purchase&lt;/div&gt;
                      </span>
            ...
            {/* Right Side: Human-Centered Explanation & Voice Narrator */}
            <div className="lg:col-span-5 space-y-6">
              
              {/* Persona Explanation Box */}
              <div className="p-6 bg-slate-950 rounded-2xl border border-slate-800 space-y-4">
                <div className="flex items-center gap-2 text-indigo-400">
                  {activePersona === 'screenreader' && <Volume2 className="w-5 h-5 shrink-0" />}
                  {activePersona === 'keyboard' && <Keyboard className="w-5 h-5 shrink-0" />}
                  {activePersona === 'contrast' && <Eye className="w-5 h-5 shrink-0" />}
                  <h4 className="text-base font-bold text-white">
                    {activePersona === 'screenreader' && 'Screen Reader Experience'}
                    {activePersona === 'keyboard' && 'Keyboard Navigation Experience'}
                    {activePersona === 'contrast' && 'Visual Contrast Experience'}
                  </h4>
                </div>
                ...
                {/* Voice Simulation Button for Screen Reader Persona */}
                {activePersona === 'screenreader' && (
                  <div className="pt-2">
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <span>{isPlayingAudio ? 'Speaking Voice Narration...' : '🔊 Click to Listen (Audio Simulation)'}</span>
                    </button>
                  </div>
                )}
              </div>

              {/* Quick Summary Benefits List */}
          ...
          {showTechCode && (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-[11px] text-slate-400">Source level AST transformation</span>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-slate-950 p-4 rounded-xl border border-rose-900/40 text-xs font-mono text-rose-300/90 overflow-x-auto">
                  <span className="text-rose-400 font-bold block mb-1">❌ Inaccessible Raw Source:</span>
  ...
  <span>Checkout</span>
                ...
                <div className="bg-slate-950 p-4 rounded-xl border border-emerald-900/40 text-xs font-mono text-emerald-300 overflow-x-auto">
                  <span className="text-emerald-400 font-bold block mb-1">✓ WCAGify Reconstructed Output:</span>
